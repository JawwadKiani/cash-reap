...
const ISSUER_BADGES: Record<string, { color: string; short: string }> = {
...
export function CardRecommendation({
  ...
  const saveMutation = useMutation({
    mutationFn: () => apiRequest("POST", `/api/saved-cards`, { cardId: card.id }),
    onSuccess: () => {
      setIsSaved(true);
  ...
  const unsaveMutation = useMutation({
    mutationFn: () => apiRequest("DELETE", `/api/saved-cards/${card.id}`),
    onSuccess: () => {
      setIsSaved(false);
      queryClient.invalidateQueries({ queryKey: [`/api/saved-cards/${user?.id}`] });
    },
  ...
  const handleSaveToggle = useCallback(() => {
    if (isSaved) {
      if (!unsaveMutation.isLoading) unsaveMutation.mutate();
    } else {
      if (!saveMutation.isLoading) saveMutation.mutate();
    }
  }, [isSaved, saveMutation, unsaveMutation]);

  const badge = ISSUER_BADGES[card.issuer] || {
    color: "bg-gray-600",
    short: card.issuer.slice(0, 4).toUpperCase(),
  };
    ...
    <div className="bg-white rounded-xl p-4 shadow-sm border border-surface-variant">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          <div
            className={`${badge.color} w-10 h-6 rounded flex items-center justify-center`}
            aria-label={`${card.issuer} badge`}
          >
        ...
        <div className="flex items-center gap-2">
          <div className="text-right">
            <div className="text-lg font-bold text-primary">
              {typeof rewardRate === "string"
                ? parseFloat(rewardRate).toFixed(0)
                : rewardRate.toFixed(0)}
              %
            </div>
          ...
          <Button
            ...
            aria-label={isSaved ? "Remove from saved cards" : "Save this card"}
      ...
      <div className="space-y-2 text-sm">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            {isRotating && (
              <span
                className="px-2 py-1 text-xs bg-yellow-400 text-yellow-900 rounded-full font-medium"
                aria-label="Active rotating category"
              >
                ACTIVE
              </span>
            )}
        ...
        <div className="flex justify-between">
          ...
          <span
            className={`text-on-surface ${
              card.annualFee === 0 ? "font-medium text-secondary" : ""
            }`}
          >
